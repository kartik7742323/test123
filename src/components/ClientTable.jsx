...
// Format a raw column value for display in filter chips / labels
function fmtColVal(col, v) {
  if (v === null || v === undefined) return '—'
  if (col.key === 'connRate' || col.key === 'qualRate') return `${Number(v).toFixed(1)}%`
  if (col.key === 'avgDurationMin') return fmtDuration(v)
  return Number(v).toLocaleString()
}

// Describe an active filter in plain English (for chips)
...
// Test a single row value against its stored filter
...
// ── Column definitions ────────────────────────────────────────────────────────
const COLS = [
  { label: 'Client Name',  key: 'name',          type: 'str', unit: ''  },
  { label: 'Use Case',     key: 'useCase',        type: 'str', unit: ''  },
  { label: 'Live Date',    key: 'liveDate',       type: 'str', unit: ''  },
  { label: 'Total Calls',  key: 'totalCalls',     type: 'num', unit: ''  },
  { label: 'Connected',    key: 'connected',      type: 'num', unit: ''  },
  { label: 'Conn Rate',    key: 'connRate',       type: 'num', unit: '%' },
  { label: 'Qualified',    key: 'qualified',      type: 'num', unit: ''  },
  { label: 'Qual Rate',    key: 'qualRate',       type: 'num', unit: '%' },
  { label: 'Avg Duration', key: 'avgDurationMin', type: 'num', unit: 'min' },
]
...
function NumericFilter({ col, data, active, onApply, onClear, onClose, alignRight }) {
  ...
  const placeholder = col.unit === '%' ? 'e.g. 35' : col.unit === 'min' ? 'decimal mins' : 'value'
    ...
    >
      ...
      <div className="px-3 pt-2.5 pb-2 border-b border-gray-100">
        ...
        <p className="text-[10px] text-gray-400 mt-0.5">
          Range: {fmtColVal(col, minV)} – {fmtColVal(col, maxV)} · Avg: {fmtColVal(col, Math.round(avg * 10) / 10)}
        </p>
      </div>
...
function TextFilter({ col, data, active, onApply, onClear, onClose, alignRight }) {
  ...
  const handleApply = () => {
    ...
    if (isAll) onClear()
    else onApply({ type: 'values', selected: [...selected] })
    onClose()
  }
    ...
    >
      ...
      <div className="px-2 pb-1.5 border-b border-gray-100">
        <label className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 rounded-lg px-2 py-1">
          <input type="checkbox" checked={allChecked} ref={el => { if (el) el.indeterminate = !allChecked && someChecked }} onChange={toggleAll} className="w-3 h-3 accent-blue-600 cursor-pointer" />
      ...
      <div className="max-h-44 overflow-y-auto p-1.5">
        ...
        {visible.map(v => (
          <label key={String(v)} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 rounded-lg px-2 py-1">
            <input type="checkbox" checked={selected.has(String(v))} onChange={() => toggle(v)} className="w-3 h-3 accent-blue-600 cursor-pointer flex-shrink-0" />
            <span className="text-xs text-gray-700 truncate">{String(v)}</span>
          </label>
        ))}
      </div>
      <div className="flex gap-2 p-2 border-t border-gray-100">
        <button onClick={() => { onClear(); onClose() }} className="flex-1 py-1.5 text-xs border border-gray-200 rounded-lg text-gray-500 hover:bg-gray-50 transition-colors">Clear</button>
        <button onClick={handleApply} className="flex-1 py-1.5 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-semibold">Apply</button>
      </div>
    </div>
  )
}
...
export default function ClientTable({ data }) {
  ...
  // 2. Column filters — numeric filters need the full dataset values for avg/top10
  ...
  // Totals row
  const avgDurMin      = sorted.length ? sorted.reduce((s, r) => s + (r.avgDurationMin || 0), 0) / sorted.length : 0
  ...
  const handleExport = () => {
    ...
    const csv = [
      headers.join(','),
      ...sorted.map(r => [r.name, r.useCase, r.liveDate, r.totalCalls, r.connected, `${r.connRate}%`, r.qualified, `${r.qualRate}%`, r.avgDuration].join(',')),
      ['Total/Average','—','—', totalRow.totalCalls, totalRow.connected, `${totalRow.connRate}%`, totalRow.qualified,'—','—'].join(','),
    ].join('\n')
    const a = Object.assign(document.createElement('a'), { href: URL.createObjectURL(new Blob([csv], { type: 'text/csv' })), download: 'client-performance.csv' })
    a.click()
  }
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5" ref={tableRef}>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
          ...
          <thead>
            <tr className="border-b-2 border-gray-100">
              {COLS.map((col, i) => {
                ...
                const alignRight = i >= COLS.length - 3
                return (
          ...
          <tbody>
            ...
            {sorted.length > 0 && (
              <tr className="border-t-2 border-gray-200 bg-gray-50 font-semibold">
                ...
                <td className="py-3 px-2 text-gray-400">—</td>
                <td className="py-3 px-2 text-amber-700 whitespace-nowrap">{totalRow.avgDuration}</td>
              </tr>
            )}
          </tbody>
        </table>
