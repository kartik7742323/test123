export default function CustomersAtRisk({ data, title = 'Customers at Risk', description = 'Inactive clients by last call date', dateLabel = 'Last Call' }) {
  if (!data) return null
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
      ...
      {total === 0 ? (
        <div className="flex items-center gap-2 py-2.5 px-3 bg-emerald-50 rounded-lg border border-emerald-100">
          ...
          <p className="text-xs text-emerald-600 font-medium">All clients active — none at risk</p>
        </div>
      ) : (
                    ...
                    {g.clients.map((c, i) => (
                      <div key={i} className="px-2.5 py-2 hover:bg-black/5 transition-colors">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          {dateLabel}: {c.lastSeen ? c.lastSeen : <span className="italic">Never</span>}
                        </p>
                      </div>
                    ))}
                  </div>
